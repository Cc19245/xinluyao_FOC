...
void bsp_enc_read_angle(void)
{
	...
	val_origin = bsp_enc_spi_rw(0xFFFF);  // 该函数返回的是uint16_t的数据
    val_tmp = val_origin << 1;  
	// 左移一位，把收到的数据的最高位丢掉，把原来的15位有符号角度数据变成现在的
	// 16位有符号数据，也就是从0x4000表示-180度，变为0x8000表示-180度
	// 也即现在得到的数据和真实的角度对应关系为-32768~+32767表示-180°~+179.99°
	code = bsp_enc_spi_rw(0xFFFF);
	
	if (ang_rx_crc8_tab_calc(val_origin) == BYTE0(code) && (code & 0x1000)) {
		mc_angle.value = val_tmp;   // .value是int16_t类型的数据
